...
export function RegisterControls({ formName, safeReturnTo }: { formName: string; safeReturnTo?: string }) {
    return function RegisterControl(form: UseFormReturn<RegisterFormValues>) {
        const {
            register,
            formState: { errors }
        } = form;
...
export function RegisterForm() {
    const onSubmit = useCallback(async (data: RegisterFormValues) => {
        const result = await registerRequest(data.username, data.email, data.password, data.verificationPassword);
        if (!result.ok) throw new Error(result.msg ?? `Could not register.`);
    }, []);
    ...
    return (
        <Modal
            ...
            defaultValues={{ email: '', username: '', password: '', verificationPassword: '' }}
        >
            {RegisterControls({ formName: 'register' })}
        </Modal>
    );
}
