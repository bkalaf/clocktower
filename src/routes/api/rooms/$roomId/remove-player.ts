// src/routes/api/rooms/$roomId/remove-player.ts
...
export const Route = createFileRoute('/api/rooms/$roomId/remove-player')({
    server: {
        handlers: {
            POST: async ({ params, request }) => {
                ...
                const payload = {
                    ...
                    type: 'memberRemoved',
                    ts: Date.now(),
                    payload: { roomId: room._id, userId: parsedBody.data.userId, action: 'removePlayer' }
                };
